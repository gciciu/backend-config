<template>
    <div>
        ...
        <mdb-row class="d-flex justify-content-center">
            <mdb-btn color="blue-grey" class="px-2" icon="arrow-up">
                 ...
                 <span v-if="props.column.field == 'id'">
                  ...
                  {{props.row.remark}}
                      ...
                      <mdb-tooltip :options="{placement: 'top'}">
                        <span slot="tip">Komponente öffnen</span>
                        <mdb-btn slot="reference" @click.native="chooseComponent(props.row)" color="geccomButtonTransparent" size="sm" icon="external-link-alt" iconRight></mdb-btn>
                      </mdb-tooltip>
                      <mdb-tooltip :options="{placement: 'top'}">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      fetchTemplates() {
        axios.get(this.restApiDomain+"/api/alltpls/"+this.customerId)
          .then((response)  =>  {
            this.loading = false;
            ...
            this.templates = JSON.parse(encodedString);
      ...
      chooseComponent(selectedComponent){
        ...
        localStorage.setItem('selectedTemplate', selectedComponent.name);
        this.$router.push(
          {
            path: '/dmgTemplate',
          }
      ...
      setCurrentSelectedTpl(selectedTpl) {
        ...
        localStorage.setItem('selectedTemplate', selectedTpl);
      },
      openComponentModal() {
      ...
      saveTemplateName() {
        ...
        axios.post(this.restApiDomain+'/api/tpls', {
          ...
          .then(function (response) {
            localStorage.setItem('selectedTemplate', response.data.name);
